(function ($) {
	Drupal.behaviors.sirius_menu = {
	  attach: function (context) {

			tableContextMenu = new ContextMenu("context-menu-id-1", menuItemClickListener);
...
function ContextMenu(contextContainerID, menuItemClickCallback, options)
{
    ...
    this.contextMenuContainer.click(function(e){
        ...
        var menuPos = {left: e.pageX, top: e.pageY};
        console.log(e);
        contextMenu.css({top: e.pageY, left: e.pageX, display: 'inline', position: 'absolute'});

        /*
        All this is broken
        if (!isElementInViewport(contextMenu) || true) {
            ...
            alert("Here with " + menuPos.top + " and left " + showPos);
        }
        else
        {
